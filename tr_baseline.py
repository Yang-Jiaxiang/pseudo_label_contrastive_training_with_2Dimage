...
#dataset selection
parser.add_argument('--dataset', type=str, default='BTS', choices=['acdc','prostate_md','mmwhs','BTS'])
...
elif parse_config.dataset == 'BTS':
    print('load BTS configs')
    import experiment_init.init_BTS as cfg
    import experiment_init.data_cfg_BTS as data_list
else:
...
if parse_config.dataset == 'acdc':
    ...
    print('set BTS orig img datalooder handle')
    orig_img_dt=dt.load_prostate_imgs_2d_as_3d
...
train_list, val_list, test_list = data_list.split_dataset_to_lists(cfg.data_path_tr_cropped, train_ratio=0.7, valid_ratio=0.15, test_ratio=0.15)
...
for epoch_i in range(start_epoch,n_epochs):
    print('train_imgs in epoch shape:', train_imgs.shape)
    print('train_labels in epoch shape:', train_labels.shape)
    ...
    print('Before shuffle_minibatch_mtask - ld_img_batch shape:', ld_img_batch.shape)

    # Apply affine transformations
    ld_img_batch,ld_label_batch=augmentation_function([ld_img_batch,ld_label_batch],dt)

    print("Before augmentation - ld_label_batch shape:", ld_label_batch.shape)
