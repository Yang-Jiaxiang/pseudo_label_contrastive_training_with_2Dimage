...
def augment_3d_vol(images,aug_en_list,aug_val_list,dt,label=0):
    #label = 0 - means image 
    #label = 1 - means mask
    if(len(aug_val_list)==0):
        rot_en,scale_en,flip_en=0,0,0
        rot_val,scale_val,flip_val=0,0,0
        aug_select=np.random.randint(3)
        if(aug_select==0):
            rot_en=1
        elif(aug_select==1):
            scale_en=1
        ...
        rot_en,scale_en,flip_en=aug_en_list[0][0],aug_en_list[0][1],aug_en_list[0][2]
        rot_val,scale_val,flip_val=aug_val_list[0][0],aug_val_list[0][1],aug_val_list[0][2]
        
    if(rot_en==1):
        if(rot_val==0):
            angles = [-30,30]
            rot_val = np.random.uniform(angles[0], angles[1])
    elif(scale_en==1):
        if(scale_val==0):
    ...
    for index in range(num_images):
        ...
        #image
        if(rot_en==1):
            img = scipy.ndimage.interpolation.rotate(img, reshape=False, angle=rot_val, axes=(1, 0),order=1)
        elif(scale_en==1):
            n_x, n_y = img.shape[0],img.shape[1]
            #print('img shape',img.shape)
            slice_rescaled = transform.rescale(img, scale_val, order=1, preserve_range=True, mode = 'constant')
            if(label==1):
                img = dt.crop_or_pad_slice_to_size_1hot(slice_rescaled, n_x, n_y)
            else:
                img = dt.crop_or_pad_slice_to_size(slice_rescaled, n_x, n_y)
        elif(flip_en==1):
            img = np.fliplr(img)
    ...
    image_aug = np.asarray(new_images)
    
    aug_en_list,aug_val_list=[],[]
    aug_en_list.append([rot_en,scale_en,flip_en])
    aug_val_list.append([rot_val,scale_val,flip_val])
    
    return image_aug,aug_en_list,aug_val_list
...
def calc_pos_neg_index(ip_mask, batch_size_ft, num_classes, no_of_pos_eles, no_of_neg_eles):
    ...
    # loop over all classes to get the elements
    for pos_index in range(0, batch_size_ft, 1):
        ...
        for cls_index in range(0, num_classes):
            # cls_where_nonzero = tf.not_equal(y_tmp_1hot[index_pos1,:,:,cls_index], zero)
            # if(tf.shape(cls_where_nonzero)[0]==0):
            #    print('0 elements for this cls label')
            #    continue
            # print('cls_index',cls_index)
            indices = np.where(ip_mask[index_pos1, :, :, cls_index] > 0)
            # print('i',pos_index,cls_index,len(indices[0]),indices)
            # break
            # if(len(indices[0])!=0):
        ...
        for cls_index_pos in range(1, num_classes):
            ...
            #     pos_ele1 = np.random.uniform(shape=[no_of_pos_eles], minval=0, maxval=tf.shape(pos_ele1_list)[0], dtype=tf.int32)
            #     pos_ele2 = np.random.uniform(shape=[no_of_pos_eles], minval=0, maxval=tf.shape(pos_ele2_list)[0], dtype=tf.int32)
            # print('pos_ele1',pos_ele1)
            ...
            # net_pos_ele1_list.append([0,cls_index_pos,pos1_tmp_list])
            n1_c = 0
